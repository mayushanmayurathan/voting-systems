...
class Election:
    ...
    def read_results(self, input_stream: TextIO) -> None:
        ...
        for data in input_stream:
            line = data.split(',')
            self.update_results(line[RIDING][1:-1], line[PARTY][1:-1],
                                int(line[VOTES]))
        ...
        for riding in self._results:
            for party in self._results[riding]:
                if self._results[riding][party] > 0:
                    if party in all_party_votes:
                        all_party_votes[party] += self._results[riding][party]
                    else:
                        all_party_votes[party] = self._results[riding][party]
    ...
    def party_seats(self) -> Dict[str, int]:
        ...
        for riding in self._results:
            for party in self._results[riding]:
                if party not in party_to_num_seats:
                    party_to_num_seats[party] = 0
            victors = self.riding_winners(riding)
            if len(victors) == 1:
                party_to_num_seats[victors[0]] += 1
        return party_to_num_seats
    ...
    def election_winners(self) -> List[str]:
        ...
        highest_victory = 0
        winner = []
        party_to_num_seats = self.party_seats()
        for party in party_to_num_seats:
            if party_to_num_seats[party] > highest_victory:
                highest_victory = party_to_num_seats[party]
                winner = [party]
            elif party_to_num_seats[party] == highest_victory:
                winner.append(party)
        return winner
...
class Jurisdiction:
    ...
    def read_results(self, year: int, month: int, day: int,
        ...
        if input_date not in self._elections:
            self._elections[input_date] = Election(input_date)
            self._elections[input_date].read_results(input_stream)
        else:
            self._elections[input_date].read_results(input_stream)
    ...
    def party_wins(self, party: str) -> List[date]:
        ...
        for dates in self._elections:
            winner = self._elections[dates].election_winners()
            if party in winner:
                date_list.append(dates)
        return date_list

        # Are you allowed to call private variables like this?
    ...
    def party_history(self, party: str) -> Dict[date, float]:
        ...
        percentage_dict = {}
        for dates in self._elections:
            all_votes = self._elections[dates].popular_vote()
            numerator = all_votes[party]
            denominator = 0
            for vote in all_votes:
                denominator += all_votes[vote]
            if denominator == 0:
                denominator = 1
            percentage_dict[dates] = numerator / denominator
    ...
    def riding_changes(self) -> List[Tuple[Set[str], Set[str]]]:
        ...
        for x in range(len(date_info) - 1):
            election_a = self._elections[date_info[x]].ridings_recorded()
            election_b = self._elections[date_info[x + 1]].ridings_recorded()
            removed = set(election_a) - set(election_b)
            added = set(election_b) - set(election_a)
